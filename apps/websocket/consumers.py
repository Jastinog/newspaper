...
class SiteConsumer(AsyncWebsocketConsumer):
    ...
    Handles analytics tracking and on-demand article summaries over one /ws/
    connection. Action routing via ACTIONS dict — each action maps to a handler.
    ...
    Client -> Server:
        ...
        {"action": "summary.generate", "article_id": 123}

    Server -> Client:
        {"type": "analytics.session",   "session_id": "uuid"}
        {"type": "summary.generating",  "article_id": 123}
        {"type": "summary.progress",    "article_id": 123, "step": 1, ...}
        {"type": "summary.ready",       "article_id": 123, "summary": "...", ...}
        {"type": "summary.error",       "article_id": 123, "message": "..."}
    """
